...
const xmlResponseTypes = [mimeTypes.xlsx, mimeTypes.xml, mimeTypes.html];
...
const responseTransformer = async function (req, res, next) {
    res.transform = async function ({ success, exportColumns, userDateFormat, isElastic, userTimezoneOffset, lookups, lookupFields, ...others }, { responseType, fileName, data: dataField = "data" } = {}) {
        ...
        const dateTimeFormat = userDateFormat + util.dateTimeExportFormat, jsonResponse = { ...others, lookups };
        ...
        if (isExportOperation) {
            ...
            if (responseType === mimeTypes.csv || responseType === mimeTypes.xlsx || responseType === mimeTypes.html) {
                data = ensureArray(data);
            }

            if (exportColumns && Object.keys(exportColumns)?.length > 0) {
                data = updateKeys({ data, keyMapping: columnKeyMappings, columns, userDateFormat, userDateTimeFormat: dateTimeFormat, userTimezoneOffset, lookups, lookupFields, isForXML: xmlResponseTypes.includes(responseType) });
            }
